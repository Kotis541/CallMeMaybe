...
class JSONStateMachine:
    ...
    def get_allowed_tokens(
        ...
        if self.state in [
            ...
            force_end = False
            if (self.current_param_type == "string" and
                    len(self.progress) > 50):
                force_end = True
            elif (self.current_param_type != "string" and
                    len(self.progress) > 10):
                force_end = True
                ...
                if self.current_param_type == "string":
                    ...
                    if '"' in token_text:
                        ...
                        if force_end:
                            ...
                            and not any(c in forbidden for c in token_text)):
                        allowed.append(int(token_id))
                else:
                    ...
                    for delim in end_tokens:
                        if delim in token_text:
                            ...
                            if all(c in valid_chars for c in prefix):
                                if (force_end or has_char
                                        or any(c.isalnum() for c in prefix)):
                                    allowed.append(int(token_id))
